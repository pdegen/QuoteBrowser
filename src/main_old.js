...
function handleFileContent(fileContent) {
  ...
  authors = [...new Set(Array.from(highlights.values()).map((entry) => entry.author))]
  authors.sort()
...
function displayHighlights(entries) {
  ...
  const totalHighlights = Object.values(groupedByBook).reduce(
    (sum, highlights) => sum + highlights[1].length,
    0,
  )
